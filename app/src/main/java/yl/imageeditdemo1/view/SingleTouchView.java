package yl.imageeditdemo1.view;
 ...
 *      其使用可以参考上面博客
 *
 *      修改:增加了deleteLocation属性,用于控制删除按钮的位置
 */
public class SingleTouchView extends View {
     ...
     * 一些默认的常量
     */
    public static final int DEFAULT_FRAME_PADDING = 8;//默认边框与图片的距离
    public static final int DEFAULT_FRAME_WIDTH = 2;//默认边框的宽度
    public static final int DEFAULT_FRAME_COLOR = Color.WHITE;//默认边框的颜色
    public static final float DEFAULT_SCALE = 1.0f;//默认缩放比例
    public static final float DEFAULT_DEGREE = 0;//默认图片旋转的角度
    public static final int DEFAULT_CONTROL_LOCATION = RIGHT_BOTTOM;//默认控制按钮的位置
    public static final int DEFAULT_DELETE_LOCATION = LEFT_TOP;//默认删除按钮的位置
    public static final boolean DEFAULT_EDITABLE = true;//默认是否可编辑
    ...
    private void init() {
        ...
        if (controlDrawable == null) {//设置控制点的默认
            controlDrawable = getContext().getResources().getDrawable(R.mipmap.ic_sticker_control);
        }
        if (deleteDrawable == null) {//设置删除点的默认
            deleteDrawable = getContext().getResources().getDrawable(R.mipmap.ic_sticker_delete);
        }
